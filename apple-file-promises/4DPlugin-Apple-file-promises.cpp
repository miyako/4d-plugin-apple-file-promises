/* --------------------------------------------------------------------------------
 #
 #  4DPlugin-Apple-file-promises.cpp
...
#include "4DPlugin-Apple-file-promises.h"
...
void sb_tell_outlook_to_export(id object) {
        ...
        if([runningApplications count])
        {
            ...
            if(application)
            {
                ...
                for (id mail in mails) {
                    ...
                    NSURL *fileUrl = [url URLByAppendingPathComponent:[NSString stringWithFormat:@"%d.%@", ++i, @"eml"]];

                    if(fileUrl) {
                        
                        [mail saveIn:fileUrl as:@"eml"];
            ...
            if(account) {
                ...
                if(mailbox) {
                    ...
                    if(fileUrl) {
                        ...
                        /*
                         only attachments respond to saveIn; not the message itself
                         */
    ...
    if (([types containsObject:kpbERMessagePasteboardType])
      ...
      ||([types containsObject:(NSString *)@"WMOutlookInternalFilePromisePboardType"])
        ...
        if(USE_PROMISE_FOR_OUTLOOK)
        {
            ...
            [swizzle_XMacNSView_saisierec performSelectorInBackground:@selector(askOutlookToExport:) withObject:object];
...
- (void)askOutlookToExport:(id)object
{
    sb_tell_outlook_to_export(object);
}
